...
In progress. See `docs/superpowers/specs/2026-05-08-distributed-workers-design.md` and the foundation + durability + runtime-services + filestore plans (`docs/superpowers/plans/2026-05-08-distributed-workers-foundation.md`, `docs/superpowers/plans/2026-05-09-distributed-workers-durability.md`, `docs/superpowers/plans/2026-05-10-distributed-workers-runtime-services.md`, `docs/superpowers/plans/2026-05-11-distributed-workers-filestore.md`). Phase 1 (foundation): claude_local + gemini_local end-to-end via gRPC, ephemeral workspaces, scope-token-bound secret fetch, lease deadline + worker-initiated keepalive, shared-secret and GCP id-token auth strategies. Phase 2 (durability): DB-backed lease + 30s reaper for control-plane-restart recovery, idempotent re-dispatch on lease expiry (spec NOTE N8), late-frame drop on re-dispatched runs, worker reconnect with exponential backoff, MIG-drain handling on the worker side. Phase 3 (runtime services): per-run dev servers / embedded postgres start on the worker, status frames flow back to update workspace_runtime_services rows, lease-expiry reaper marks orphaned services stopped on worker death. Phase 4 (filestore mode): opt-in per-project flag for shared-filesystem workspaces; concurrent runs serialize via the workspace_leases table whose partial unique index `WHERE released_at IS NULL` is the actual lock; workspace-lease reaper releases orphaned rows on worker death.
