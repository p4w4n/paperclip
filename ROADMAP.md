...
Five-phase distributed-workers spec landed. See `docs/superpowers/specs/2026-05-08-distributed-workers-design.md` and the five plans under `docs/superpowers/plans/2026-05-{08..12}-distributed-workers-*.md`. Phase 1 (foundation): claude_local + gemini_local end-to-end via gRPC, ephemeral workspaces, scope-token-bound secret fetch, lease deadline + worker-initiated keepalive, shared-secret and GCP id-token auth. Phase 2 (durability): DB-backed lease + 30s reaper, idempotent re-dispatch on lease expiry, late-frame drop on re-dispatched runs, worker reconnect with exponential backoff, MIG-drain handling. Phase 3 (runtime services): per-run dev servers / embedded postgres on the worker, status frames flow back to workspace_runtime_services. Phase 4 (filestore mode): opt-in per-project shared-filesystem workspaces, workspace_leases partial-unique lock serializes concurrent runs. Phase 5 (GCP polish): admin /api/_workers + UI, manual + SIGTERM auto-drain, worker-metrics for the MIG autoscaler custom metric, Cloud Monitoring publisher, GCS storage provider, signed-URL session blob redirect for large adapter sessions.
